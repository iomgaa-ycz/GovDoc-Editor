...
- Use exactly one `Bash` call to invoke `node "${CLAUDE_PLUGIN_ROOT}/scripts/codex-companion.mjs" task --cwd "$(pwd)" ...`.
- **CRITICAL: Always pass `--cwd "$(pwd)"` to the companion script.** This ensures Codex operates in the current working directory (which may be a git worktree, not the main repository root). Without this flag, Codex defaults to the main repo and commits land on the wrong branch.
