...
export function workpaperToHtml(wp: WorkpaperPayload): string {
  ...
  if (wp.findings.length > 0) {
    ...
    for (const f of wp.findings) {
      ...
      const verdictBadge = `<span style="display:inline-block;padding:1px 8px;border-radius:4px;color:${vc};background:${vbg};font-weight:600;">${escapeHtml(v.verdict)}</span>`;
      // 问题列：标题 + 判定徽章 + 严重程度/分类
