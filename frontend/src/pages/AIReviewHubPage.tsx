import { Eye, FileCheck, FileText, FolderOpen, Info, Loader2, Plus, X } from "lucide-react";
import { useEffect, useMemo, useState, type FormEvent } from "react";
import { useNavigate } from "react-router-dom";

import { createAuditRun, createProject, listAuditRuns, listCheckpoints, listProjects } from "@/api/v3";
...
import type { AuditRun, CheckpointItem, GovCheckpointPayload, Project } from "@/types/ui";
...
function AIReviewDrawer({ open, onClose, onCreated }: AIReviewDrawerProps) {
  ...
  async function handlePickerConfirm(selectedIds: string[]): Promise<void> {
    ...
    try {
      if (pickerTarget === "main") {
        ...
        setSuppDocs((prev) => deduplicateDocuments([...prev, ...selected]));
      }
      setPickerOpen(false);
    } catch (err) {
