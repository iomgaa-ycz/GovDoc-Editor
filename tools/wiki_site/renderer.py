...
def serve(
    ...
    if config_path is None:
        default_cfg = project_root / "tools" / "wiki-site" / "wiki-site.config.yaml"
    ...
    vite_cmd = [str(vite_bin)] if vite_bin.exists() else ["npx", "vite"]
    vite_proc = subprocess.Popen(
        [*vite_cmd, "--host", "--port", str(config.port)],
        cwd=str(app_dir),
    )
